...
hello_counter(struct nk_context* ctx, struct media* media, GLFWwindow* win)
{
    ...
    if (!show_popup_window) {
        ...
        if (nk_begin(ctx, "Pop-up window", nk_rect(WINDOW_WIDTH / 2 - 300, WINDOW_HEIGHT / 2 - 175, 600, 350),
            NK_WINDOW_TITLE | NK_WINDOW_BORDER | NK_WINDOW_MOVABLE | NK_WINDOW_NO_SCROLLBAR | NK_WINDOW_MINIMIZABLE | NK_WINDOW_SCALABLE))
        {
