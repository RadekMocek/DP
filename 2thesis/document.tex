...
\begin{document}
	...
	Navrhovanou ukázkovou aplikací této práce je program pro tvorbu a editaci diagramů. Diagram je celkem obecný pojem a v kontextu této práce se myslí především vývojový diagram, nebo blokové schéma.
	
	Navrhovaná aplikace na základě zadaného textového popisu vykreslí odpovídající diagram. Dělí se tedy na dvě hlavní části: víceřádkové textové pole a kreslicí plátno. Textové pole je základní ovládací prvek, který je ale relativně komplexní, a proto je vhodné jej použít jako jeden z hlavních stavebních kamenů aplikace. Plátno pak pomůže prozkoumat schopnost knihoven vykreslovat programátorem definované objekty. Tedy vykreslování i něčeho jiného, než jsou základní ovládací prvky, které knihovna poskytuje.
	...
	Ukázková aplikace této práce se snaží o unikátní přístup k tomuto problému. Diagram je také generován na základě dodaného kódu, tento kód ovšem popisuje, jak by měl diagram přesně vypadat. Formát SVG v podstatě také popisuje přesný vzhled a export do tohoto formátu je jediný způsob, jak může uživatel vykreslit svůj diagram mimo navrhovaný program. Přímé použití formátu SVG pro popis diagramu je ale nevhodné. Z důvodu své všestrannosti a syntaxi jazyka XML má tento formát vyšší  režijní náklady ve smyslu délky kódu, který je potřeba pro popsání libovolného diagramu.
	...
	Uzel je ve své základní podobě obdélník obsahující text. Pro tvorbu jednoduchého blokového schématu je potřeba mít pro každý uzel možnost specifikovat jeho umístění, velikost a textový obsah. Pro snadnější editaci existujících diagramů je zde požadavek, aby bylo možné definovat pozici uzlu relativně k jinému uzlu. Tím vznikají vztahy typu \textit{rodič-potomek} a při změně pozice rodičovského uzlu se změní pozice i všech jeho potomků. Další požadavek je, aby bylo možné uzly definovat v libovolném pořadí, tedy aby mohl být ve zdrojovém textu potomek definován dříve než jeho rodič.
	
	Pozice uzlu je určena nastavením hodnot pro klíče \textit{x} a \textit{y}, které odpovídají souřadnicím kreslicího plátna. Co ale přesně souřadnice pro daný uzel znamenají? Mohl by zde být umístěn jeho střed, nebo také některý z rohů vykresleného obdélníka. Proto jsem přidal klíč \textit{pivot}, kde lze toto chování pro každý uzel nastavit. Hodnotou tohoto klíče je řetězec, např. \texttt{"center"} nebo \texttt{"top-left"}. V doménově specifickém jazyce by bylo možné mít pro tento účel dedikovaná klíčová slova, pak by byl neplatný \textit{pivot} zachycen již během parsování. V tomto případě je nutné kontrolovat správnost argumentu až v pozdější fázi.
	
	Otázka ohledně přesného umístění vykreslených uzlů nastává také u relativních souřadnic. Pokud by souřadnice potomka byly relativní např. ke středu rodiče, byla by vyřešena problematika přesunů, ale ne změn velikosti. Příkladem je diagram s dvěma uzly ve vztahu rodič-potomek, kde je rodičův pivot nastaven na střed. Uživatel si přeje, aby byl potomek umístěn pět jednotek pod svým rodičem. Změna pozice rodiče by toto zachovala, ale po změně jeho výšky by bylo nutné zvýšit ypsilonovou souřadnici potomka, protože se střed rodiče nijak neposunul. Proto jsem přidal možnost definovat pivota rodiče, ke kterému jsou souřadnice potomka relativní. Obrázek \ref{_tag_img_TOMLSchema} ukazuje popsaný scénář a také prozrazuje další vlastnosti navrženého schématu.
	
	\begin{figure}[ht]
		\centering
		\includegraphics[width=\textwidth]{img/toml_schema}
		\caption{Schéma pro popis diagramu v jazyce TOML}
		\label{_tag_img_TOMLSchema}
	\end{figure}

	Definice vztahů rodič-potomek fungují na bázi identifikátorů typu textového řetězce. Potomek uvádí identifikátor a pivot rodiče, který se tak stává počátkem pro jeho souřadnice. Dále je vidět, že pokud není pro uzel nastavena velikost explicitně, použije se velikost implicitní, která je určená textovým popiskem uzlu.
	
	* Po testech byla navržena verze 2, protože\dots
	
	* Pak se také ukázalo, že by bylo vhodné mít pro každou osu rodiče zvlášť, ale\dots
	
	* Jaké další [NODE] atributy máme
