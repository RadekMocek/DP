...
\begin{document}
	...
	Udržovat si určité informace je potřeba i z důvodu podpory komplexnějšího rozložení ovládacích prvků. Příkladem může být dialogové okno, které se má objevit přesně uprostřed hlavního okna a které si automaticky určuje svou velikost podle svého obsahu. Je-li jeho obsahem jedno tlačítko, pak šířka tohoto tlačítka určuje šířku celého dialogového okna. Aby mohlo být dialogové okno umístěno doprostřed okna hlavního, je nejprve potřeba znát jeho velikost\footnote{Pokud se umístění prvků specifikuje souřadnicemi levého horního rohu, pak je horizontální souřadnice prvku, který je umístěn uprostřed rodičovského prvku, vypočítána odečtením poloviny šířky prvku od poloviny šířky rodiče.}, pro kterou je zase potřeba znát šířku tlačítka. Tlačítko je proto nutné vykreslit, v této části kódu ale ještě není známo jeho umístění.
	
	Problému nenapomáhá ani fakt, že textový popisek tlačítka je dynamický a může se tedy měnit každý snímek. \cite{online_eguidocs}
