...
\begin{document}
	...
	\begin{abbrList2}
		...
		\textbf{SVG} & Scalable Vector Graphics, standard pro popis vektorové grafiky \\
		\textbf{WPF} & Windows Presentation Foundation, knihovna umožňující tvorbu GUI aplikací pro operační systém Windows \\
		\textbf{WYSIWYG} & What You See Is What You Get; typ programu, kde má editovaný dokument stejnou podobu jako výsledný dokument \\
		\textbf{WYSIWYM} & What You See Is What You Mean; typ programu, kde je výsledný dokument vygenerován z editovaného dokumentu podle určitých pravidel \\
		\textbf{XML} & Extensible Markup Language, jazyk popisující strukturu dat
	\end{abbrList2}
	...
	Navrhovanou ukázkovou aplikací této práce je program pro tvorbu a editaci diagramů. Diagram je celkem obecný pojem, v kontextu této práce se ale myslí především vývojový diagram, nebo blokové schéma.
	...
	Při programování GUI aplikací se ale často pracuje právě se základními ovládacími prvky, a proto by je ukázková aplikace měla využívat. Nemělo by se však jednat o „galerii ovládacích prvků“, kterou lze často najít jako příklad v dokumentaci dané knihovny. Použité ovládací prvky by měly dávat smysl v kontextu editoru diagramů. Proto navrhovaná aplikace obsahuje např. celočíselné vstupní pole pro změnu velikosti písma, slider pro změnu přiblížení pohledu na diagram, nebo výběr barvy (\textit{color picker}) pro změnu barvy uzlu bez nutnosti zásahu do zdrojového textu daného diagramu.
	...
	Navrhovaná aplikace dále disponuje funkcemi, které by uživatel od programu tohoto typu mohl očekávat. Tvorba diagramu zde spočívá v editaci textového řetězce a je na místě, aby měl uživatel možnost jej uložit nebo nahrát ze souboru. Proto je k dispozici lišta nabídek obsahující akce, které pro tento účel vyvolají nativní dialogové okno. Záhlaví hlavního okna pak obsahuje cestu k právě editovanému souboru a zároveň indikuje neuložené změny pomocí znaku hvězdičky. Při pokusu o zavření aplikace nebo nahrání jiného souboru je v případě neuložených změn uživatel vyzván k potvrzení, zdali chce změny uložit nebo zahodit. Další akcí v liště nabídek je vyvolání okna pro nastavení. Zde lze především přepnout vzhled aplikace do tmavého režimu, což je mezi uživateli často žádanou funkcí \cite{inpro_darkmode}. Aplikace by také měla bez kompromisů podporovat operační systémy Windows a Linux.
	...
	Existující aplikace, které umožňují tvorbu diagramů, lze rozdělit do dvou kategorií. První jsou programy využívající přístup WYSIWYG, kde uživatel interaguje přímo s objekty diagramu vykreslenými na plátně. Do této kategorie se řadí např. program \textit{draw.io}. Druhou kategorií je přístup WYSIWYM. V něm uživatel píše kód, který popisuje, co by měl daný diagram vyobrazovat. Do této kategorie patří např. program \textit{Mermaid}. Jelikož kód popisuje především význam daného diagramu, nemá uživatel velkou kontrolu nad jeho vzhledem. Kód může např. popisovat vztahy mezi jednotlivými uzly, jejich přesné umístění v diagramu už ale určuje samotný program.
	...
	Nejideálnější by bylo použít doménově specifický jazyk, který by byl od začátku navržen pro účely této aplikace.
	
	\begin{itemize}
		\item Nejideálnější by bylo navrhnout DSL, proč se tak nestalo.
		\item Proč byl zvolen TOML.
		\item Dva stavební prvky \textit{node} a \textit{path}. Jaké jsou na ně požadavky.
	\end{itemize}
