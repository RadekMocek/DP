...
\begin{document}
	...
	\begin{abbrList2}
		...
		%\textbf{WYSIWYM} & What You See Is What You Mean, přístup k tvorbě strukturovaných dokumentů
	\end{abbrList2}
	...
	Ukázkovou aplikací této práce je editor diagramů, který na základě zadaného textového popisu vykreslí odpovídající diagram. Aplikace se tedy dělí na dvě hlavní části: víceřádkové textové pole a kreslicí plátno. Textové pole je jeden ze základních ovládacích prvků, který je ale relativně komplexní, a proto je vhodné jej použít jako jeden z hlavních stavebních kamenů aplikace. Plátno pak pomůže prozkoumat schopnost knihoven vykreslovat programátorem definované objekty.
	
	*
	
	* vývoj je GUI však převážně o widgetech, nechceme widget gallery, ale chceme widgety
	
	\begin{itemize}
		\item Editor diagramů, který na základě dodaného textového popisu vykreslí odpovídající diagram.
		\item $\implies$ Dvě hlavní části: víceřádkové textové pole pro popis diagramu a plátno pro jeho vykreslení.
		\item $\implies$ Otestování komplexnějšího widgetu a funkcí pro vykreslování 2D primitiv.
		\item Dále přidávat funkce, které využijí dalších dostupných ovládacích prvků a tím prohloubí poznání dané knihovny. Nebo funkce, které jsou od desktopových GUI aplikací tohoto typu očekávané. Tyto funkce explicitně vypsat.
		\item Nemělo by se jednat pouze o „widget gallery“. Krom toho, že by měla aplikace sloužit jako ukázka a prostředek pro porovnání daných knihoven, by se měla také blížit k reálně použitelnému programu pro tvorbu diagramů. Proto je na místě export do SVG a kompatibilita s OS Windows i Linux.
	\end{itemize}
	
	\section{Princip popisu diagramu pomocí textu}
	
	\begin{itemize}
		\item Pokus o unikátní přístup, kde se nepopisuje, co má diagram znamenat, ale jak má přesně vypadat.
